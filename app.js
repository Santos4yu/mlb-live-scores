...
function renderGameTab(data){
    ...
    if(isLive&&pitches.length>0){
        ...
        const pitchDots=visiblePitches.map((p,pi)=>{
            ...
            return`<div class="mini-sz-dot ${pcls}" data-x="${px}" data-y="${py}" data-eid="${p.eventId||pi}" data-latest="${isLatest}" style="left:${isLatest?90:px}px;top:${isLatest?90:py}px;${isLatest?'opacity:0':''}">${p.pitchNumber||''}</div>`;
        }).join('');
